...
const libraryManager = (function () {
  ...
  const cacheDOM = (function () {
    ...
    const inputTitleError = document.querySelector("#title + span.error");
    const inputAuthorError = document.querySelector("#author + span.error");
  ...
  function bindEvents() {
    ...
    cacheDOM.cancelBtn.addEventListener("click", () =>
      cacheDOM.bookDialog.close()
    );

    // return value = id of current book that user is editing. if empty = new book.
    cacheDOM.bookDialog.addEventListener("submit", (e) => {
      if (
        !cacheDOM.inputTitle.validity.valid ||
        !cacheDOM.inputAuthor.validity.valid
      ) {
        checkForError(cacheDOM.inputTitle, cacheDOM.inputTitleError);
        checkForError(cacheDOM.inputAuthor, cacheDOM.inputAuthorError);
        e.preventDefault();
      } else {
    ...
    cacheDOM.inputTitle.addEventListener("input", () =>
      checkForError(cacheDOM.inputTitle, cacheDOM.inputTitleError)
    );
    cacheDOM.inputAuthor.addEventListener("input", () =>
      checkForError(cacheDOM.inputAuthor, cacheDOM.inputAuthorError)
    );
  }

  function checkForError(inputItem, inputError) {
    if (inputItem.validity.valid) {
      inputError.textContent = "";
    } else {
      if (inputItem.validity.valueMissing) {
        inputError.textContent = "You need to enter this field!";
      } else if (inputItem.validity.tooShort) {
        inputError.textContent = "The entered value is too short!";
      }
    }
  }
    ...
    libraryArr.forEach((book) => {
      ...
      for (const [key, value] of Object.entries(book)) {
        ...
        if (key === "read") {
          ...
          if (value === "") newCell.textContent = "N/A";
          else {
      ...
      removeButton.addEventListener("click", () => {
        libraryArr.splice(
          libraryArr.findIndex((item) => item.id === book.id),
          1
        );
        updateLibraryTable();
      });
